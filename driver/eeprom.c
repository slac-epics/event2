...
void eeprom_cmd_start(volatile struct Pci9030LocalConf *pLC)
{
...
void eeprom_cmd_stop(volatile struct Pci9030LocalConf *pLC)
{
...
int eeprom_wait(volatile struct Pci9030LocalConf *pLC)
{
  int timeout_jiffies;
...
int eeprom_write_bits(volatile struct Pci9030LocalConf *pLC,
...
int eeprom_read_bits(volatile struct Pci9030LocalConf *pLC,
...
int eeprom_read(struct Pci9030LocalConf *pLC,
		unsigned short *eepromdata, unsigned int wordcount)
{
  ...
  eeprom_cmd_start(pLC);
  eeprom_write_bits(pLC, 0x600, 11);

  /* Now DO should be low, to show that EEPROM is there */
  eeprom_read_bits(pLC, &eeword, 1);
  if (eeword & 1)
    {
  ...
  for (address = 0; address < wordcount; address++)
    {
      eeprom_read_bits(pLC, &eeword, 16);
...
int eeprom_sprint(struct Pci9030LocalConf *pLC, char *buf, int size)
{
  ...
  result = eeprom_read(pLC, eepromdata, EEPROM_MAX_WORDS);
  ...
  for (i = 0; EepromComment[i] != NULL; i++)
    if (s < size)
      s += snprintf(&buf[s], size-s, "0x%04x %s\n",
		    eepromdata[i], EepromComment[i]);
...
int eeprom_write(struct Pci9030LocalConf *pLC,
		 unsigned short *eepromdata, unsigned int wordcount)
{
  int address, eeword;
  ...
  /* EWEN */
  eeprom_cmd_start(pLC);
  eeprom_write_bits(pLC, 0x4C0, 11);
  eeprom_cmd_stop(pLC);
  ...
  for (address = 0; address < wordcount; address++)
    {
      ...
      eeprom_cmd_start(pLC);
      eeprom_write_bits(pLC, 0x700 | (address), 11);
      eeprom_cmd_stop(pLC);
      eeprom_cmd_start(pLC);
      timeout = eeprom_wait(pLC);
      if (timeout)
	result = timeout;
      eeprom_cmd_stop(pLC);
      ...
      /* Now program */
      eeprom_cmd_start(pLC);
      eeprom_write_bits(pLC, 0x500 | (address), 11);
      eeword = eepromdata[address];
      eeprom_write_bits(pLC, eeword, 16);
      eeprom_cmd_stop(pLC);
      eeprom_cmd_start(pLC);
      timeout = eeprom_wait(pLC);
      if (timeout)
  ...
  /* EWDS */
  eeprom_cmd_start(pLC);
  eeprom_write_bits(pLC, 0x400, 11);
  eeprom_cmd_stop(pLC);
...
#ifdef DEBUG
  printk(KERN_INFO "eeprom_sccan read %d words pLC = %08x.\n", datalen,
	 (unsigned int) pLC);
#endif

  if (datalen)
    {
      result = eeprom_write(pLC, eepromdata, datalen);
      if (!result)
	result = datalen;
    }
